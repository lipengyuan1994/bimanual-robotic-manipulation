...
def export_skill_corrective_dataset(view_path, store: EvidenceStore, destination, repo_id) -> Path:
    ...
    views = load_skill_corrective_views(view_path, store)
    ...
    try:
        ...
        (destination / "skill_corrective_views.json").write_bytes(original_view)
        ...
        if (
            load_skill_corrective_views(destination / "skill_corrective_views.json", copied)
            != views
        ):
        ...
        if (
            load_skill_corrective_views(view_path, store) != views
        ...
        payload = dict(
            ...
            storage="images",
            split="train",
            task_scope="bounded_component_replay_only",
            independent_scene_count=None,
            frames=sum(s.end - s.start for s in views.sources),
            episodes=_episodes(views),
            camera_features=CAMERA_FEATURES,
            joint_order=list(JOINT_ORDER),
            skill_corrective_views_sha256=digest_file(destination / "skill_corrective_views.json"),
            files={
...
def verify_skill_corrective_dataset(root: Path) -> dict:
    ...
    views = load_skill_corrective_views(root / "skill_corrective_views.json", store)
    if payload.get("skill_corrective_views_sha256") != digest_file(
        root / "skill_corrective_views.json"
    ):

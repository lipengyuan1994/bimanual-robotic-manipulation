...
def _native():
    if platform.system() == "Darwin" and platform.machine() != "arm64":
        raise RuntimeError("Local hand-off continuity export requires native Apple Silicon Python")
    ...
    for index, source in enumerate(views.sources):
        ...
        rows.append(
            dict(
                ...
                parent_validation_end=source.validation_end,
...
def _parity(dataset, views, store):
    ...
    if dataset.num_frames != expected_count or dataset.num_episodes != len(views.sources):
        raise ValueError("Hand-off continuity export frame/episode count mismatch")
    offset = 0
        ...
        for local, expected in enumerate(_frames(store.directory(source.run_id), source)):
            ...
            for key, expected_index in (
                ...
                if (
                    ...
                    raise ValueError("Hand-off continuity export index mapping mismatch")
            if actual["task"] != expected["task"]:
                raise ValueError("Hand-off continuity export instruction mismatch")
...
def export_handoff_continuity_dataset(
    view_path, store: EvidenceStore, destination, repo_id
) -> Path:
    ...
    if not isinstance(repo_id, str) or not re.fullmatch(r"[A-Za-z0-9_-]+/[A-Za-z0-9_-]+", repo_id):
        raise ValueError("Invalid hand-off continuity dataset repo_id")
    destination, view_path = Path(destination).absolute(), Path(view_path)
    if destination.exists() or destination.is_symlink():
        raise FileExistsError("Hand-off continuity dataset destination already exists")
    views = load_handoff_continuity_views(view_path, store)
    ...
    try:
        ...
        (destination / "handoff_continuity_views.json").write_bytes(original_view)
        ...
        if (
            load_handoff_continuity_views(destination / "handoff_continuity_views.json", copied)
            != views
        ):
            raise ValueError("Copied hand-off continuity source changed")
        for source in views.sources:
        ...
        if (
            load_handoff_continuity_views(view_path, store) != views
            or view_path.read_bytes() != original_view
        ):
            raise ValueError("Hand-off continuity input changed during export")
        payload = dict(
            schema_version=1,
            profile=PROFILE,
            format="lerobot_v3",
            ...
            storage="images",
            split="train",
            task_scope="receiver_close_shared_hold_donor_release",
            independent_scene_count=None,
            frames=sum(s.end - s.start for s in views.sources),
            episodes=_episodes(views),
            camera_features=CAMERA_FEATURES,
            joint_order=list(JOINT_ORDER),
            handoff_continuity_views_sha256=digest_file(
                destination / "handoff_continuity_views.json"
            ),
            files={
...
def _require_offline_hub():
    ...
    if os.environ.get("HF_HUB_OFFLINE") != "1" or not constants.HF_HUB_OFFLINE:
        raise RuntimeError(
            "Start hand-off continuity verification with HF_HUB_OFFLINE=1 before imports"
        )


def verify_handoff_continuity_dataset(root: Path) -> dict:
    ...
    if hashlib.sha256(canonical(body)).hexdigest() != payload.get("manifest_sha256"):
        raise ValueError("Hand-off continuity export manifest digest mismatch")
    expected = dict(
        schema_version=1,
        profile=PROFILE,
        format="lerobot_v3",
    ...
    if any(key not in payload or payload[key] != value for key, value in expected.items()):
        raise ValueError("Unsupported hand-off continuity export profile")
    ...
    if not files or actual != set(files) or "EXPORT_FAILED.json" in actual:
        raise ValueError("Incomplete or unsealed hand-off continuity dataset")
    for name, sha in files.items():
        Artifact(path=name, sha256=sha).verify(root)
    store = EvidenceStore(root / "raw_sources")
    views = load_handoff_continuity_views(root / "handoff_continuity_views.json", store)
    if payload.get("handoff_continuity_views_sha256") != digest_file(
        root / "handoff_continuity_views.json"
    ):
        raise ValueError("Hand-off continuity view binding mismatch")
    if payload.get("episodes") != _episodes(views) or payload.get("frames") != sum(
        s.end - s.start for s in views.sources
    ):
        raise ValueError("Hand-off continuity export source/index mapping mismatch")
    if "meta/info.json" not in files or not any(
        name.startswith("data/") and name.endswith(".parquet") for name in files
    ):
        raise ValueError("Hand-off continuity dataset requires sealed local metadata and data")

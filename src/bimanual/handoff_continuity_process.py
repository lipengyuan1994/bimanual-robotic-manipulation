"""Guardian-owned process boundary for one hand-off continuity collection case."""
...
from bimanual.handoff_continuity_teacher import KIND as CONTINUITY_KIND
from bimanual.handoff_continuity_teacher import (
    _existing,
    build_handoff_continuity_request,
...
PROCESS_KIND = "handoff_receiver_continuity_teacher_recording_process"


class HandoffContinuityProcessConfig(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, allow_inf_nan=False)
    protocol_path: Path
    case_id: str = Field(min_length=1)
    wall_timeout_seconds: float = Field(default=1800.0, gt=0, le=7200)
...
def _continuity_child(
    ...
    try:
        ...
        config = HandoffContinuityProcessConfig.model_validate(config_data)
...
def _resolve(config: HandoffContinuityProcessConfig, project_root: Path):
    return config.model_copy(
        update={"protocol_path": (project_root / config.protocol_path).resolve(strict=True)}
    )


def _confine(config: HandoffContinuityProcessConfig, store: EvidenceStore):
    if config.protocol_path.is_relative_to(store.root):
        raise ValueError("Process evidence must be outside the immutable continuity protocol")


def run_handoff_continuity_process(
    config: HandoffContinuityProcessConfig,
    *,
    store: EvidenceStore,
    project_root: Path,
    cancelled: Callable[[], bool] = lambda: False,
    _entrypoint: Callable = run_handoff_continuity_case,
    _request_builder: Callable = build_handoff_continuity_request,
) -> Manifest:
    """Run one identity-reserved case with bounded cancellation and parent-loss cleanup."""

    config = HandoffContinuityProcessConfig.model_validate(config.model_dump(mode="json"))
    ...
    expected_request = _request_builder(config.protocol_path, config.case_id)
    # A busy worker is not a collection attempt. Reserve the shared lease before
    ...
    with WorkerLease.acquire(store.root / ".handoff-continuity-coordinator.lock"):
        prior = _existing(store, expected_request)
    ...
    try:
        if os.name != "posix":
            raise RuntimeError("Continuity collection guardian requires POSIX")
        ...
        if prestart_cancelled or cancelled():
            ...
            process = _PinnedGuardian(
                ...
                name="bimanual-handoff-continuity-guardian",
                daemon=False,
            )
    ...
    try:
        ...
        if isinstance(message, dict) and isinstance(message.get("run_id"), str):
            ...
            try:
                ...
                if child.kind != CONTINUITY_KIND or child.config != expected_request:
                    raise ValueError("Child manifest kind/config does not match continuity case")
                if (
                    child.metrics.get("teacher_assistance") is not True
                    or child.metrics.get("learned_execution") is not False
                    or child.metrics.get("release_qualified") is not False
                ):
                    raise ValueError("Child continuity result exceeds its training-only scope")
                passed = child.metrics.get("physical_handoff_success") is True
                if (child.outcome == "completed") != passed:
                    raise ValueError("Child outcome contradicts independent continuity score")
                score = child.metrics.get("independent_score")
                if passed and (
                    not isinstance(score, dict)
                    or score.get("profile") != "handoff_receiver_continuity_physical_score_v1"
                    or score.get("physical_handoff_success") is not True
                ):
                    raise ValueError("Child success lacks its independent continuity score")
                metrics.update(
                    child_manifest_verified=True,
                    child_manifest_sha256=child.manifest_sha256,
                    child_outcome=child.outcome,
                    physical_handoff_success=passed,
                )
                if (
                    reason is None
        ...
        if reason is not None or metrics["child_exitcode"] not in (None, 0):
            metrics.update(
                state=reason or "failed",
                process_complete=False,
                physical_handoff_success=False,
            )
        if metrics["forced_interruption"]:
            metrics.update(process_complete=False, physical_handoff_success=False)
        ...
        return store.seal(
            ...
            source=source,
            claims=(
                ["Verified bounded process completion for one training-only continuity case"]

...
| R8 | Reproducible repository, simulation, benchmark, video, architecture | [Setup](SETUP.md), [architecture](ARCHITECTURE.md), [sealed evidence/index and verification](EVIDENCE.md), [read-only portal](DEPLOYMENT.md), source/asset lineage, physical replay bundles and automated tests are implemented. The [local release declaration](WORKFLOW_RELEASE.md) can freeze a real seven-checkpoint candidate, Qwen, all sixteen scene inputs, runtime sources and execution settings before evaluation. | Create that declaration after training, execute and aggregate the full-task suite, verify clean installation and rollback, then finish benchmark, video and submission artifacts. The learning site and isolated replays are not the dinner-task application. |

...
def test_late_workbench_overlap_profile_binds_fresh_cases_and_full_bar_skill_window():
    protocol = module.BarOverlapCorrectionProtocol.model_validate(
        _body(
            "bar_late_workbench_overlap_protocol_v8",
            source_interval=(630, 1580),
            case_seeds=(59000, 59001, 59002, 59003, 59004),
        )
    )
...
def test_late_workbench_overlap_profile_requires_the_exact_sealed_failure_signature():
    component = {
        ...
        "recorded_autonomous_skill_actions": 255,
        "confirmed_action_log_actions": 255,
        "rejected_action_log_actions": 1,
        "maximum_overlap_m": 0.0026645663252162573,
        "maximum_overtravel_m": 0.0,
        "maximum_target_displacement_m": 0.16014351093651127,
    ...
    diagnosis = SimpleNamespace(
        ...
        outcome="completed",
        manifest_sha256=module.LATE_WORKBENCH_OVERLAP_FAILURE_ANALYSIS_MANIFEST_SHA256,
    ...
    accepted, bound = module._verified_diagnosis(
        Store(), "diagnosis", profile="bar_late_workbench_overlap_protocol_v8"
    )
    assert accepted is diagnosis
    assert bound is evaluation

    component["overlap_peak"] = {"contacts": []}

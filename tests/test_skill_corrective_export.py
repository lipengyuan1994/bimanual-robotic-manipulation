...
import numpy as np
import pytest
from test_skill_corrective_views import _source as source

from bimanual import skill_corrective_export as module
...
def inputs(tmp_path, monkeypatch):
    ...
    run = source(store, monkeypatch)
    path = tmp_path / "views.json"
    create_skill_corrective_views(store, [run.run_id], path)
    return store, path


def test_export_verify_and_unchanged_intervention(tmp_path, mock_library, monkeypatch):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_skill_corrective_dataset(
        path, store, tmp_path / "dataset", "local/corrective"
    )
    payload = module.verify_skill_corrective_dataset(root)
    assert payload["frames"] == 2
    assert [(e["dataset_start"], e["dataset_end"]) for e in payload["episodes"]] == [(0, 2)]
    assert [e["parent_start"] for e in payload["episodes"]] == [3]
    raw = root / payload["episodes"][0]["raw_root"] / "demonstration/episode.json"
    assert json.loads(raw.read_text())["interventions"] == 1
    assert len(json.loads(raw.read_text())["frames"]) == 6
    with pytest.raises(FileExistsError):
        module.export_skill_corrective_dataset(path, store, root, "local/corrective")


def test_failed_parity_retained_unsealed(tmp_path, mock_library, monkeypatch):
    store, path = inputs(tmp_path, monkeypatch)
...
def test_verifier_rejects_resealed_index_mapping(tmp_path, mock_library, monkeypatch):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_skill_corrective_dataset(
        path, store, tmp_path / "dataset", "local/corrective"
    )
    ...
    with pytest.raises(ValueError, match="mapping"):
        module.verify_skill_corrective_dataset(root)


def test_verifier_checks_decoded_rows_again(tmp_path, mock_library, monkeypatch):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_skill_corrective_dataset(
        path, store, tmp_path / "dataset", "local/corrective"
    )
...
def test_native_guard_before_creation(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="native"):
        module.export_skill_corrective_dataset(
            "missing", None, tmp_path / "dataset", "local/corrective"
        )
    assert not (tmp_path / "dataset").exists()


@pytest.mark.parametrize("value", [0.5, float("nan"), float("inf"), True])
def test_fractional_and_invalid_index_rejected(tmp_path, mock_library, value, monkeypatch):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_skill_corrective_dataset(
        path, store, tmp_path / "dataset", "local/corrective"
    )

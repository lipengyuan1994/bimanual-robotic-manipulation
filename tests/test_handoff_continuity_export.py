"""Mock optional LeRobot; verify continuity mapping and failure preservation."""
...
import numpy as np
import pytest
from test_handoff_continuity_views import _source
...
def inputs(tmp_path, monkeypatch):
    ...
    create_handoff_continuity_views(store, [source.run_id], path)
    return store, path


def test_export_verify_and_unchanged_intervention(tmp_path, monkeypatch, mock_library):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_handoff_continuity_dataset(
        path, store, tmp_path / "dataset", "local/continuity"
    )
    payload = module.verify_handoff_continuity_dataset(root)
    assert payload["frames"] == 2
    assert [(e["dataset_start"], e["dataset_end"]) for e in payload["episodes"]] == [(0, 2)]
    assert [e["parent_start"] for e in payload["episodes"]] == [2]
    ...
    with pytest.raises(FileExistsError):
        module.export_handoff_continuity_dataset(path, store, root, "local/continuity")


def test_failed_parity_retained_unsealed(tmp_path, monkeypatch, mock_library):
    store, path = inputs(tmp_path, monkeypatch)
...
def test_verifier_rejects_resealed_index_mapping(tmp_path, monkeypatch, mock_library):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_handoff_continuity_dataset(
        path, store, tmp_path / "dataset", "local/continuity"
    )
    ...
    with pytest.raises(ValueError, match="mapping"):
        module.verify_handoff_continuity_dataset(root)


def test_verifier_checks_decoded_rows_again(tmp_path, monkeypatch, mock_library):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_handoff_continuity_dataset(
        path, store, tmp_path / "dataset", "local/continuity"
    )
...
def test_native_guard_before_creation(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="native"):
        module.export_handoff_continuity_dataset(
            "missing", None, tmp_path / "dataset", "local/continuity"
        )
    assert not (tmp_path / "dataset").exists()


@pytest.mark.parametrize("value", [0.5, float("nan"), float("inf"), True])
def test_fractional_and_invalid_index_rejected(tmp_path, monkeypatch, mock_library, value):
    store, path = inputs(tmp_path, monkeypatch)
    root = module.export_handoff_continuity_dataset(
        path, store, tmp_path / "dataset", "local/continuity"
    )
